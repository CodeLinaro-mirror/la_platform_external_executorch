...
Algorithm: iterative argmax/argmin with masking, adapted from
FlagGems / aiter topk (1-stage path for moderate row sizes).
"""
...
def _topk_kernel(
    ...
    for j in tl.static_range(0, K):
        if LARGEST:
            vsel = tl.max(vals, axis=0)
        else:
            vsel = tl.min(vals, axis=0)
        ...
        tl.store(out_v_ptr + j, vsel)
        tl.store(out_i_ptr + j, arg)

        # Mask out the selected element
        vals = tl.where(idxs == arg, FILL, vals)
