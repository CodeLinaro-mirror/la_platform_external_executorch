...
def _producer_nodes(node: torch.fx.Node) -> set[torch.fx.Node]:
    ...
    while queue:
        ...
        for argument in pytree.tree_leaves((current.args, current.kwargs)):
...
def _mutable_buffer_get_attrs(
    ...
    for node in graph_module.graph.nodes:
        if not _is_mutation_node(node):
            continue
        mutation_target = cast(torch.fx.Node, node.args[0])
        get_attrs.update(
            producer
            for producer in _producer_nodes(mutation_target)
            if producer.op == "get_attr" and isinstance(producer.target, str)
        )
    return get_attrs
