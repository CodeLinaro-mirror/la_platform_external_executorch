...
class TestBatchNorm2d(unittest.TestCase):
    ...
    # Expected to fail since ArmTOSAQuantizer cannot quantize a BatchNorm layer
    # TODO(MLETORCH-100)
    @parameterized.expand(test_data_suite)
    @unittest.skip(
        reason="Expected to fail since ArmTOSAQuantizer (for BI) cannot quantize a BatchNorm layer"
    )
    ...
    # Expected to fail since ArmEthosUQuantizer (ArmTOSAQuantizer (BI)) cannot quantize a BatchNorm layer
    # TODO(MLETORCH-100)
    @parameterized.expand(test_data_suite)
    @unittest.skip(
        reason="Expected to fail since ArmEthosUQuantizer cannot quantize a BatchNorm layer"
    )
    @unittest.expectedFailure
