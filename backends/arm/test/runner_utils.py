...
class TosaReferenceModelDispatch(TorchFunctionMode):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if not self.ran_tosa_dispatch:
            raise RuntimeError(
                "Ran model with TosaReferenceModelDispatch but never ran ArmTOSABackend delegate."
            )
    ...
    def __torch_function__(self, func, types, args=..., kwargs=None):
        if func is torch._higher_order_ops.executorch_call_delegate:
            ...
            if lowered_backend_module.backend_id == "ArmTOSABackend":
                self.ran_tosa_dispatch = True
                return self._tosa_dispatch(lowered_backend_module, args[1:])
            else:
                raise RuntimeError(
                    f"Ran model with TosaReferenceModelDispatch but call_delegate with {lowered_backend_module.backend_id=} != 'ArmTOSABackend'."
                )
