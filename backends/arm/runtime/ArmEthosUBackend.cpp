...
class ArmEthosUBackend final : public ::executorch::runtime::BackendInterface {
 public:
  ...
  Error execute(
    ...
    EXECUTORCH_PROF_SCOPE(event_tracer, "ArmEthosUBackend::execute()");
    ...
    EXECUTORCH_PROF_START(
        ...
        "+ArmEthosUBackend::execute()processed_data");
    ...
    EXECUTORCH_PROF_START(
        event_tracer,
        event_tracer_local_scope,
        "+ArmEthosUBackend::execute()vela_bin_read()");
    ...
    for (int i = 0; i < handles.inputs->count; i++) {
      ...
      if (both_char and permuted_input_shape) {
        EXECUTORCH_PROF_SCOPE(
            event_tracer,
            "+ArmEthosUBackend::execute()handles.input.permute_CHW_to_HWC()");
        ...
        EXECUTORCH_PROF_SCOPE(
            event_tracer, "+ArmEthosUBackend::execute()handles.input.memcpy()");
    ...
    for (int i = 0; i < handles.outputs->count; i++) {
      ...
      if (tensor_out.scalar_type() == ScalarType::Char and
        ...
        EXECUTORCH_PROF_SCOPE(
            event_tracer,
            "+ArmEthosUBackend::execute()handles.output.permute_HWC_to_CHW()");
        ...
        EXECUTORCH_PROF_SCOPE(
            event_tracer, "+ArmEthosUBackend::execute()handles.output.move()");
