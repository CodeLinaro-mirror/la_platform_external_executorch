...
#ifdef EXECUTORCH_BUILD_CUDA
  auto mutable_state = std::make_unique<
      ::executorch::backends::cuda::MutableStateContextOwner>();
  if (Error e = register_mutable_fqns(meta_module.get(), *mutable_state);
      e != Error::Ok) {
    return e;
  }
#endif
...
#ifdef EXECUTORCH_BUILD_CUDA
  auto module_res = mutable_state->with_load_scope(
...
#ifdef EXECUTORCH_BUILD_CUDA
  rebind_available = mutable_state->available();
  if (rebind_available) {
