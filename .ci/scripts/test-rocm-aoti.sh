...
TORCHAO_ROCM_INDEX="${TORCHAO_ROCM_INDEX:-https://download.pytorch.org/whl/nightly/rocm${ROCM_VERSION}}"
...
python -m pip install -r requirements-dev.txt \
  ...
  --extra-index-url "${TORCHAO_ROCM_INDEX}"
...
assert arch == "gfx1100", f"Expected gfx1100, got {device.gcnArchName}"
assert device.warp_size == 32, f"Expected wave32, got {device.warp_size}"
...
ROCM_EXCLUDED_TESTS=(
  ...
  # Defer specialized SDPA suites until their gfx1100 cost is measured.
...
python - "${ROCM_POINTWISE_DIR}" <<'PY'
import pathlib
import sys

import numpy as np
import torch
from examples.cuda.scripts.export_amd_pointwise import AmdPointwiseModule

output_dir = pathlib.Path(sys.argv[1])
x = np.linspace(-3.0, 3.0, 1024 * 1024, dtype=np.float32).reshape(1024, 1024)
expected = AmdPointwiseModule()(torch.from_numpy(x)).numpy()
x.tofile(output_dir / "input.bin")
expected.astype(np.float32).tofile(output_dir / "expected.bin")
PY

"${EXECUTORCH_EXECUTOR_RUNNER}" \
  --model_path="${ROCM_POINTWISE_DIR}/amd_triton.pte" \
  --data_path="${ROCM_POINTWISE_DIR}/aoti_cuda_blob.ptd" \
  --inputs="${ROCM_POINTWISE_DIR}/input.bin" \
  --output_file="${ROCM_POINTWISE_DIR}/output"
...
output_dir = pathlib.Path(sys.argv[1])
expected = np.fromfile(output_dir / "expected.bin", dtype=np.float32)
actual = np.fromfile(output_dir / "output-0.bin", dtype=np.float32)
np.testing.assert_allclose(actual, expected, rtol=1e-4, atol=1e-5)
print("ROCm AOTI pointwise output matches the reference")
PY
